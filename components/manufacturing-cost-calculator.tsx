  ...
  return <section className="manufacturing-panel" id="manufacturing-costs" aria-labelledby="manufacturing-cost-title">
    ...
    <div className="manufacturing-cost-grid">
      ...
      <div className="manufacturing-chart" role="img" aria-label={t("manufacturing.volumeChartLabel")}>
        ...
        <ResponsiveContainer width="100%" height={260}><LineChart data={volumeData} margin={{ top: 10, right: 12, left: 8, bottom: 8 }}><CartesianGrid stroke="rgba(56,189,248,.12)" /><XAxis dataKey="label" stroke="#94a3b8" /><YAxis stroke="#94a3b8" tickFormatter={formatChartInr} /><Tooltip formatter={(value) => money.format(Number(value))} contentStyle={{ background: "#081426", border: "1px solid rgba(56,189,248,.25)" }} /><Legend /><Line type="monotone" dataKey="minimum" name="Minimum" stroke="#38bdf8" /><Line type="monotone" dataKey="typical" name="Typical" stroke="#10b981" /><Line type="monotone" dataKey="maximum" name="Maximum" stroke="#22d3ee" /></LineChart></ResponsiveContainer>
      </div>
      <div className="manufacturing-chart" role="img" aria-label={t("manufacturing.categoryChartLabel")}>
        ...
        <ResponsiveContainer width="100%" height={260}><BarChart data={summary.categoryTotals} layout="vertical" margin={{ top: 8, right: 12, left: 24, bottom: 8 }}><CartesianGrid stroke="rgba(56,189,248,.12)" /><XAxis type="number" stroke="#94a3b8" tickFormatter={formatChartInr} /><YAxis type="category" dataKey="category" width={88} stroke="#94a3b8" tick={{ fontSize: 10 }} /><Tooltip formatter={(value) => money.format(Number(value))} contentStyle={{ background: "#081426", border: "1px solid rgba(56,189,248,.25)" }} /><Bar dataKey="typicalPerUnit" name="Typical per unit" fill="#10b981" /></BarChart></ResponsiveContainer>
      </div>
    </div>
