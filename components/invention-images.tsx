  ...
  return <section className="images-section">
    <div className="image-upload-card card">
      ...
      <form action={action} className="image-upload-form">
        ...
        <label className="file-drop"><input name="images" type="file" accept="image/jpeg,image/png,image/webp" multiple required /><span>＋</span><strong>Choose images</strong><small>JPG, PNG, or WebP · maximum 10 MB each · up to 10 at once</small></label>
        <label className="image-type-field"><span>Image type</span><select name="image_type" defaultValue="" required><option value="" disabled>Select a type</option>{imageTypes.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
        {state.error && <div className="image-action-error" role="alert">{state.error}</div>}
        <button className="upload-button" type="submit" disabled={pending}>{pending && <span className="auth-spinner" aria-hidden="true" />}{pending ? "Uploading…" : "Upload images"}</button>
      </form>
    </div>
