...
describe("curated grant matching", () => {
  ...
  it("caps a missing applicant profile", () => { const incomplete = rankSchemes(CURATED_GOVERNMENT_SCHEMES, detected, { ...applicant, applicantType: "not_sure", dpiitRecognised: "not_sure", udyamRegistered: "not_sure", incorporated: "not_sure", incorporatedUnderTwoYears: "not_sure", hasPrototype: "not_sure" }); assert.equal(Math.max(...incomplete.map((item) => item.score)) <= 74, true); });
  ...
  it("caps unresolved hard requirements", () => { const sisfs = rankSchemes(CURATED_GOVERNMENT_SCHEMES, detected, { ...applicant, applicantType: "dpiit_startup", dpiitRecognised: "yes", incorporatedUnderTwoYears: "not_sure" }).find((item) => item.scheme.id === "sisfs"); assert.ok(sisfs); assert.equal(sisfs.score <= 69, true); assert.equal(sisfs.eligibilityStatus, "CHECK_REQUIREMENTS"); });
  ...
  it("applies hard requirements before numeric relevance", () => { const denied = rankSchemes(CURATED_GOVERNMENT_SCHEMES, detected, { ...applicant, applicantType: "dpiit_startup", dpiitRecognised: "no", incorporatedUnderTwoYears: "yes" }); const sisfs = denied.find((item) => item.scheme.id === "sisfs"); assert.ok(sisfs); assert.equal(sisfs.score <= 39, true); });
...
describe("grant UI logic and privacy", () => {
  ...
  it("clears stale live sections when applicant information changes", () => { const original = { curated: matches, live: [matches[0]], needsVerification: [matches[1]], detectedProfile: classifyInvention(pillBox), liveEnabled: true, liveCheckedAt: "2026-07-26T10:00:00.000Z", notice: null }; const refreshed = recalculateCuratedForApplicant(original, { ...applicant, dpiitRecognised: "yes" }); assert.equal(refreshed.live.length, 0); assert.equal(refreshed.needsVerification.length, 0); assert.equal(refreshed.liveCheckedAt, null); assert.equal(refreshed.curated.length > 0, true); });
