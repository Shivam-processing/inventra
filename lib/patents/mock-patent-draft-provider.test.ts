...
import { MockPatentDraftProvider } from "./mock-patent-draft-provider";
...
test("generates a clean deterministic patent draft from stored disclosure data", async () => {
  ...
  assert.match(draft.preliminaryClaims, /3\. The apparatus of claim 2/);
  ...
  assert.ok(abstractWords >= 60 && abstractWords <= 150, `abstract contained ${abstractWords} words`);
});
